...
public interface IVmsRepository extends JpaRepository<Vms,Serializable> {
   ...
   public Long toggleStatus(@Param("status") String status, @Param("id") Long id,@Param("rem") String rem);
	
	
   @Query("SELECT c FROM Vms c  WHERE c.addedby = :id")
   public List<Vms> searchbyid(@Param("id") int id);
   ...
   @Query("SELECT new com.narvee.usit.entity."
	   		+ "Vms(v.id,v.company_name,v.recruiter_name,v.cp_mobile,v.cpemail,v.headQuarters,v.vendortype,u.firstname,v.createddate,v.status) from Vms v,Users u where v.addedby=u.userid and CONCAT(v.company_name,  v.recruiter_name, v.cp_mobile, v.cpemail, v.headQuarters, v.vendortype, v.status,  v.addedby,  v.createddate) LIKE %?1%") 
